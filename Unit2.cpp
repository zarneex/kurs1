...
void __fastcall TForm2::N8Click(TObject *Sender)
{
    ...
    if (StringGrid1->RowCount > 1 && !fileOpened) {
        // Если есть данные и файл не открыт, добавляем строку в конец
        ...
        StringGrid1->Cells[0][newRow] = IntToStr(newRow - 1); // Номер строки (учитывая шапку)
        StringGrid1->Cells[1][newRow] = ""; // Наименование товара
        StringGrid1->Cells[2][newRow] = ""; // Цена
        StringGrid1->Cells[3][newRow] = ""; // Количество
        StringGrid1->Cells[4][newRow] = ""; // Срок годности
        StringGrid1->Cells[5][newRow] = ""; // Дата изготовления
        StringGrid1->Cells[6][newRow] = ""; // Дата поступления

        // Устанавливаем выделение на новую строку
        StringGrid1->Row = newRow;
        StringGrid1->Col = 1;

        StatusBar1->SimpleText = "Добавлена новая строка в конец";
    } else {
        // Если таблица пустая или файл открыт, добавляем строку в начало
        StringGrid1->RowCount++; // Увеличиваем количество строк на 1
        ...
        // Заполняем новую первую строку
        StringGrid1->Cells[0][1] = "1"; // Номер строки
        StringGrid1->Cells[1][1] = ""; // Наименование товара
        StringGrid1->Cells[2][1] = ""; // Цена
        StringGrid1->Cells[3][1] = ""; // Количество
        StringGrid1->Cells[4][1] = ""; // Срок годности
        StringGrid1->Cells[5][1] = ""; // Дата изготовления
        StringGrid1->Cells[6][1] = ""; // Дата поступления
        ...
        StatusBar1->SimpleText = "Добавлена новая строка в начало";
    }

	// Очищаем поля ввода в другой форме, если они связаны с таблицей
    Form1->Edit1->Clear();
    Form1->Edit2->Clear();
    Form1->Edit3->Clear();
    Form1->Edit4->Clear();
...
void __fastcall TForm2::N4Click(TObject *Sender)
{
	...
	if (myfile.is_open()) // Проверка успешного открытия файла
	{
		...
		myfile.close(); // закрытие файла
		StatusBar1->SimpleText = ("Данные успешно сохранены в файл.");
	}
	else
	{
...
void __fastcall TForm2::N3Click(TObject *Sender) {
    ...
    if (myfile.is_open()) { // Проверка успешного открытия файла
        ...
        myfile.close(); // Закрытие файла
		StatusBar1->SimpleText = ("Данные успешно сохранены в файл: " + fileName);
    } else {
...
void TForm2::SortTable(int column) {
	...
	StatusBar1->SimpleText = "Сортировка завершена.";
}
...
{
	//Справка
	TFileStream *fileStream = new TFileStream("Справка.txt", fmOpenRead);
	try {
