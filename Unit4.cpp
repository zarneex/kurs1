...
{
    static bool isFiltered = false; // Переменная для отслеживания состояния фильтрации
    static TStringList *originalRows = nullptr; // Для хранения исходных строк

    if (!isFiltered) {
        // Сохраняем исходные данные в первый раз
        if (!originalRows) {
        ...
        try {
            ...
            Form2->StringGrid1->RowCount = 1; // Очищаем строки (кроме заголовка)
